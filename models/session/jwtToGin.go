...
algorithm、sessions：每次认证通过以后，服务器需要创建一条记录保存用户信息，通常是在内存中，随着认证通过的用户越来越多，服务器在保存session的开销会越来越大
...
jwt与oauth的区别：
algorithm、oauth2是一个授权框架，jwt是一个认证协议
