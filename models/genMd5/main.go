...
func main() {
	...
	salt, encodedPwd := password.Encode("algorithm password", option)

	check := password.Verify("algorithm password", salt, encodedPwd, option)
	fmt.Println(check)
	...
	//验证密码
	check2 := password.Verify("algorithm password", passwordInfo[2], passwordInfo[3], option)
