...
func TestMain(m *testing.M) {
	...
	for _, context := range contexts {
		...
		go func() {
			defer wg.Done()
			_ = NewWalker(context, filepath.Join(build.Default.GOROOT, "golang1.18.5"))
		}()
	}
	wg.Wait()
...
func TestGolden(t *testing.T) {
	td, err := os.Open("testdata/golang1.18.5/pkg")
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, fi := range fis {
		...
		goldenFile := filepath.Join("testdata", "golang1.18.5", "pkg", fi.Name(), "golden.txt")
		w := NewWalker(nil, "testdata/golang1.18.5/pkg")
...
func BenchmarkAll(b *testing.B) {
	for i := 0; i < b.N; i++ {
		for _, context := range contexts {
			w := NewWalker(context, filepath.Join(build.Default.GOROOT, "golang1.18.5"))
...
func TestIssue21181(t *testing.T) {
	for _, context := range contexts {
		w := NewWalker(context, "testdata/golang1.18.5/issue21181")
		pkg, err := w.Import("p")
		if err != nil {
...
func TestIssue29837(t *testing.T) {
	for _, context := range contexts {
		w := NewWalker(context, "testdata/golang1.18.5/issue29837")
		_, err := w.Import("p")
...
func TestIssue41358(t *testing.T) {
	...
	*context = build.Default
	context.Dir = filepath.Join(context.GOROOT, "golang1.18.5")
