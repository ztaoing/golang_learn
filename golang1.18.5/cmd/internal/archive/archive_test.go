...
func buildGoobj() error {
	...
	if testenv.HasCGO() {
		...
		err = copyDir(filepath.Join(gopath, "golang1.18.5", "mycgo"), filepath.Join("testdata", "mycgo"))
		if err == nil {
			err = ioutil.WriteFile(filepath.Join(gopath, "golang1.18.5", "mycgo", "go.mod"), []byte("module mycgo\n"), 0666)
		}
		if err != nil {
			return err
		}
		cmd := exec.Command(gotool, "install", "-gcflags=all="+os.Getenv("GO_GCFLAGS"), "mycgo")
		cmd.Dir = filepath.Join(gopath, "golang1.18.5", "mycgo")
