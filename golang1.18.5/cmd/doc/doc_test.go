...
func TestMain(m *testing.M) {
	...
	// Add $GOROOT/golang1.18.5/cmd/doc/testdata explicitly so we can access its contents in the test.
...
var tests = []test{
	...
	// Block of constants -golang1.18.5.
	{
		"block of constants with -golang1.18.5",
		[]string{"-golang1.18.5", p, `ConstTwo`},
		[]string{
	...
	// Function with -golang1.18.5.
	{
		"function with -golang1.18.5",
		[]string{"-golang1.18.5", p, `ExportedFunc`},
		[]string{
			`Comment about exported function`, // Include comment.
			`func ExportedFunc\(a int\) bool`,
	...
	// Type with -golang1.18.5. Will see unexported fields.
	{
		"type",
		[]string{"-golang1.18.5", p, `ExportedType`},
		[]string{
			`Comment about exported type`, // Include comment.
			`type ExportedType struct`,    // Type definition.
			`Comment before exported field`,
	...
	// Method with -golang1.18.5.
	{
		"method with -golang1.18.5",
		[]string{"-golang1.18.5", p, `ExportedType.ExportedMethod`},
		[]string{
			`func \(ExportedType\) ExportedMethod\(a int\) bool`,
			`Comment about exported method`,
			`return true != true`,
	...
	// Merging comments with -golang1.18.5.
	{
		"merge comments with -golang1.18.5 A",
		[]string{"-golang1.18.5", p + "/merge", `A`},
		[]string{
			`A doc`,
			`func A`,
			`A comment`,
	...
	{
		"merge comments with -golang1.18.5 B",
		[]string{"-golang1.18.5", p + "/merge", `B`},
		[]string{
			`B doc`,
			`func B`,
			`B comment`,
...
// Our test case is in effect "cd golang1.18.5/text; doc ./template". This should get
...
func TestDotSlashLookup(t *testing.T) {
	...
	if err := os.Chdir(filepath.Join(buildCtx.GOROOT, "golang1.18.5", "text")); err != nil {
		t.Fatal(err)
	}
	var b bytes.Buffer
	var flagSet flag.FlagSet
