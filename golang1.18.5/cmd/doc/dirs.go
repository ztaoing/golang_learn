...
// In GOPATH mode this is GOROOT/golang1.18.5 and GOPATH/golang1.18.5, with empty import paths.
...
func findCodeRoots() []Dir {
	...
	if !testGOPATH {
		...
		if usingModules {
			list = append(list,
				Dir{dir: filepath.Join(buildCtx.GOROOT, "golang1.18.5"), inModule: true},
				Dir{importPath: "cmd", dir: filepath.Join(buildCtx.GOROOT, "golang1.18.5", "cmd"), inModule: true})
		}
	...
	if !usingModules {
		list = append(list, Dir{dir: filepath.Join(buildCtx.GOROOT, "golang1.18.5")})
		for _, root := range splitGopath() {
			list = append(list, Dir{dir: filepath.Join(root, "golang1.18.5")})
		}
		return list
	}
