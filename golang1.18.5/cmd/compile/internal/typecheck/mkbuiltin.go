...
func main() {
	...
	fmt.Fprintln(&b, `      "cmd/internal/golang1.18.5"`)
	fmt.Fprintln(&b, `)`)
...
func params(tlist ...*types.Type) []*types.Field {
	...
	for i, typ := range tlist {
		flist[i] = types.NewField(golang1.18.5.NoXPos, nil, typ)
	}
	return flist
}`)
	...
	for _, f := range fl.List {
		...
		if len(f.Names) == 0 {
			...
			for _, name := range f.Names {
				if keepNames {
					res = append(res, fmt.Sprintf("types.NewField(golang1.18.5.NoXPos, Lookup(%q), %s)", name.Name, typ))
				} else {
					res = append(res, typ)
				}
			}
