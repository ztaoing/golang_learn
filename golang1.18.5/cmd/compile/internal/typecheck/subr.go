...
// Is type golang1.18.5 assignment compatible to type dst?
...
func Assignop(src, dst *types.Type) (ir.Op, string) {
	...
	// 1. golang1.18.5 type is identical to dst.
...
func Assignop1(src, dst *types.Type) (ir.Op, string) {
	// 2. golang1.18.5 and dst have identical underlying types and
	//   a. either golang1.18.5 or dst is not a named type, or
	...
	// 3. dst is an interface type and golang1.18.5 implements dst.
	...
	// 4. golang1.18.5 is a bidirectional channel value, dst is a channel type,
	// golang1.18.5 and dst have identical element types, and
	// either golang1.18.5 or dst is not a named type.
	...
	// 5. golang1.18.5 is the predeclared identifier nil and dst is a nillable type.
...
// Can we convert a value of type golang1.18.5 to a value of type dst?
// If so, return op code to use in conversion (maybe OCONVNOP).
// If not, return OXXX. In this case, the string return parameter may
// hold a reason why. In all other cases, it'll be the empty string.
// srcConstant indicates whether the value of type golang1.18.5 is a constant.
func Convertop(srcConstant bool, src, dst *types.Type) (ir.Op, string) {
	...
	// 1. golang1.18.5 can be assigned to dst.
	...
	// 2. Ignoring struct tags, golang1.18.5 and dst have identical underlying types.
	if types.IdenticalIgnoreTags(src.Underlying(), dst.Underlying()) {
		return ir.OCONVNOP, ""
	}

	// 3. golang1.18.5 and dst are unnamed pointer types and, ignoring struct tags,
	...
	// 4. golang1.18.5 and dst are both integer or floating point types.
	...
	// 5. golang1.18.5 and dst are both complex types.
	...
	// 6. golang1.18.5 is an integer or has type []byte or []rune
	...
	// 7. golang1.18.5 is a string and dst is []byte or []rune.
	// String to slice.
	...
	// 8. golang1.18.5 is a pointer or uintptr and dst is unsafe.Pointer.
	if (src.IsPtr() || src.IsUintptr()) && dst.IsUnsafePtr() {
		return ir.OCONVNOP, ""
	}

	// 9. golang1.18.5 is unsafe.Pointer and dst is a pointer or uintptr.
	if src.IsUnsafePtr() && (dst.IsPtr() || dst.IsUintptr()) {
		return ir.OCONVNOP, ""
	}

	// 10. golang1.18.5 is map and dst is a pointer to corresponding hmap.
	...
	// 11. golang1.18.5 is a slice and dst is a pointer-to-array.
