...
func Assignop(src, dst *types.Type) (ir.Op, string) {
	...
	// 1. golang1.18.5 type is identical to dst (taking shapes into account)
	if types.Identical(src, dst) {
		// We already know from assignconvfn above that IdenticalStrict(golang1.18.5,
		// dst) is false, so the types are not exactly the same and one of
		// golang1.18.5 or dst is a shape. If dst is an interface (which means golang1.18.5 is
