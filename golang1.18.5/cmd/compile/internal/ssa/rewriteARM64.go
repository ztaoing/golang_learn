...
func rewriteValueARM64_OpMove(v *Value) bool {
	...
	// result: (MOVBstore dst (MOVBUload golang1.18.5 mem) mem)
	for {
		if auxIntToInt64(v.AuxInt) != 1 {
			break
		}
	...
	// result: (MOVHstore dst (MOVHUload golang1.18.5 mem) mem)
	for {
	...
	// result: (MOVWstore dst (MOVWUload golang1.18.5 mem) mem)
	for {
	...
	// result: (MOVBstore [2] dst (MOVBUload [2] golang1.18.5 mem) (MOVHstore dst (MOVHUload golang1.18.5 mem) mem))
	for {
	...
	// result: (MOVBstore [4] dst (MOVBUload [4] golang1.18.5 mem) (MOVWstore dst (MOVWUload golang1.18.5 mem) mem))
	for {
	...
	// result: (MOVHstore [4] dst (MOVHUload [4] golang1.18.5 mem) (MOVWstore dst (MOVWUload golang1.18.5 mem) mem))
	for {
	...
	// result: (MOVBstore [6] dst (MOVBUload [6] golang1.18.5 mem) (MOVHstore [4] dst (MOVHUload [4] golang1.18.5 mem) (MOVWstore dst (MOVWUload golang1.18.5 mem) mem)))
	for {
	...
	// result: (MOVWstore [8] dst (MOVWUload [8] golang1.18.5 mem) (MOVDstore dst (MOVDload golang1.18.5 mem) mem))
	for {
	...
	// match: (Move [24] dst golang1.18.5 mem)
	...
	// cond: s%8 != 0 && s > 8
	// result: (Move [s%8] (OffPtr <dst.Type> dst [s-s%8]) (OffPtr <golang1.18.5.Type> golang1.18.5 [s-s%8]) (Move [s-s%8] dst golang1.18.5 mem))
	for {
	...
	// result: (MOVDstore [int32(s-8)] dst (MOVDload [int32(s-8)] golang1.18.5 mem) (DUFFCOPY <types.TypeMem> [8*(64-(s-8)/16)] dst golang1.18.5 mem))
	for {
		s := auxIntToInt64(v.AuxInt)
		dst := v_0
		src := v_1
	...
	// result: (DUFFCOPY [8 * (64 - s/16)] dst golang1.18.5 mem)
	for {
		s := auxIntToInt64(v.AuxInt)
		dst := v_0
		src := v_1
	...
	// result: (LoweredMove dst golang1.18.5 (ADDconst <golang1.18.5.Type> golang1.18.5 [s-8]) mem)
	for {
		s := auxIntToInt64(v.AuxInt)
		dst := v_0
		src := v_1
...
func rewriteValueARM64_OpSelectN(v *Value) bool {
	...
	b := v.Block
	config := b.Func.Config
	// match: (SelectN [0] call:(CALLstatic {sym} s1:(MOVDstore _ (MOVDconst [sz]) s2:(MOVDstore _ golang1.18.5 s3:(MOVDstore {t} _ dst mem)))))
	// cond: sz >= 0 && isSameCall(sym, "runtime.memmove") && s1.Uses == 1 && s2.Uses == 1 && s3.Uses == 1 && isInlinableMemmove(dst, golang1.18.5, sz, config) && clobber(s1, s2, s3, call)
	...
	// match: (SelectN [0] call:(CALLstatic {sym} dst golang1.18.5 (MOVDconst [sz]) mem))
