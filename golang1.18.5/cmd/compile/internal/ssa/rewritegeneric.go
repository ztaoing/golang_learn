...
func rewriteValuegeneric_OpMove(v *Value) bool {
	...
	b := v.Block
	config := b.Func.Config
	// match: (Move {t} [n] dst1 golang1.18.5 mem:(Zero {t} [n] dst2 _))
	// cond: isSamePtr(golang1.18.5, dst2)
	...
	// match: (Move {t} [n] dst1 golang1.18.5 mem:(VarDef (Zero {t} [n] dst0 _)))
	// cond: isSamePtr(golang1.18.5, dst0)
	// result: (Zero {t} [n] dst1 mem)
	for {
		n := auxIntToInt64(v.AuxInt)
		t := auxToType(v.Aux)
	...
	// match: (Move {t1} [s] dst tmp1 midmem:(Move {t2} [s] tmp2 golang1.18.5 _))
	// cond: t1.Compare(t2) == types.CMPeq && isSamePtr(tmp1, tmp2) && isStackPtr(golang1.18.5) && !isVolatile(golang1.18.5) && disjoint(golang1.18.5, s, tmp2, s) && (disjoint(golang1.18.5, s, dst, s) || isInlinableMemmove(dst, golang1.18.5, s, config))
	// result: (Move {t1} [s] dst golang1.18.5 midmem)
	for {
	...
	// match: (Move {t1} [s] dst tmp1 midmem:(VarDef (Move {t2} [s] tmp2 golang1.18.5 _)))
	// cond: t1.Compare(t2) == types.CMPeq && isSamePtr(tmp1, tmp2) && isStackPtr(golang1.18.5) && !isVolatile(golang1.18.5) && disjoint(golang1.18.5, s, tmp2, s) && (disjoint(golang1.18.5, s, dst, s) || isInlinableMemmove(dst, golang1.18.5, s, config))
	// result: (Move {t1} [s] dst golang1.18.5 midmem)
	for {
		s := auxIntToInt64(v.AuxInt)
	...
	// match: (Move dst golang1.18.5 mem)
	// cond: isSamePtr(dst, golang1.18.5)
	// result: mem
	for {
		dst := v_0
		src := v_1
...
func rewriteValuegeneric_OpSelectN(v *Value) bool {
	...
	// match: (SelectN [0] call:(StaticCall {sym} s1:(Store _ (Const64 [sz]) s2:(Store _ golang1.18.5 s3:(Store {t} _ dst mem)))))
	// cond: sz >= 0 && isSameCall(sym, "runtime.memmove") && t.IsPtr() && s1.Uses == 1 && s2.Uses == 1 && s3.Uses == 1 && isInlinableMemmove(dst, golang1.18.5, int64(sz), config) && clobber(s1, s2, s3, call)
	// result: (Move {t.Elem()} [int64(sz)] dst golang1.18.5 mem)
	for {
		if auxIntToInt64(v.AuxInt) != 0 {
			break
		}
	...
	// match: (SelectN [0] call:(StaticCall {sym} s1:(Store _ (Const32 [sz]) s2:(Store _ golang1.18.5 s3:(Store {t} _ dst mem)))))
	// cond: sz >= 0 && isSameCall(sym, "runtime.memmove") && t.IsPtr() && s1.Uses == 1 && s2.Uses == 1 && s3.Uses == 1 && isInlinableMemmove(dst, golang1.18.5, int64(sz), config) && clobber(s1, s2, s3, call)
	// result: (Move {t.Elem()} [int64(sz)] dst golang1.18.5 mem)
	for {
		if auxIntToInt64(v.AuxInt) != 0 {
	...
	// match: (SelectN [0] call:(StaticCall {sym} dst golang1.18.5 (Const64 [sz]) mem))
	// cond: sz >= 0 && call.Uses == 1 && isSameCall(sym, "runtime.memmove") && dst.Type.IsPtr() && isInlinableMemmove(dst, golang1.18.5, int64(sz), config) && clobber(call)
	// result: (Move {dst.Type.Elem()} [int64(sz)] dst golang1.18.5 mem)
	for {
		if auxIntToInt64(v.AuxInt) != 0 {
			break
		}
	...
	// match: (SelectN [0] call:(StaticCall {sym} dst golang1.18.5 (Const32 [sz]) mem))
	// cond: sz >= 0 && call.Uses == 1 && isSameCall(sym, "runtime.memmove") && dst.Type.IsPtr() && isInlinableMemmove(dst, golang1.18.5, int64(sz), config) && clobber(call)
	// result: (Move {dst.Type.Elem()} [int64(sz)] dst golang1.18.5 mem)
	for {
		if auxIntToInt64(v.AuxInt) != 0 {
	...
	// match: (SelectN [0] call:(StaticLECall {sym} dst golang1.18.5 (Const64 [sz]) mem))
	// cond: sz >= 0 && call.Uses == 1 && isSameCall(sym, "runtime.memmove") && dst.Type.IsPtr() && isInlinableMemmove(dst, golang1.18.5, int64(sz), config) && clobber(call)
	// result: (Move {dst.Type.Elem()} [int64(sz)] dst golang1.18.5 mem)
	for {
		if auxIntToInt64(v.AuxInt) != 0 {
	...
	// match: (SelectN [0] call:(StaticLECall {sym} dst golang1.18.5 (Const32 [sz]) mem))
	// cond: sz >= 0 && call.Uses == 1 && isSameCall(sym, "runtime.memmove") && dst.Type.IsPtr() && isInlinableMemmove(dst, golang1.18.5, int64(sz), config) && clobber(call)
	// result: (Move {dst.Type.Elem()} [int64(sz)] dst golang1.18.5 mem)
	for {
		if auxIntToInt64(v.AuxInt) != 0 {
...
func rewriteValuegeneric_OpStore(v *Value) bool {
	...
	// match: (Store {t} dst (Load golang1.18.5 mem) mem)
	// cond: !fe.CanSSA(t)
	// result: (Move {t} [t.Size()] dst golang1.18.5 mem)
	for {
		t := auxToType(v.Aux)
		dst := v_0
		if v_1.Op != OpLoad {
	...
	// match: (Store {t} dst (Load golang1.18.5 mem) (VarDef {x} mem))
	// cond: !fe.CanSSA(t)
	// result: (Move {t} [t.Size()] dst golang1.18.5 (VarDef {x} mem))
	for {
		t := auxToType(v.Aux)
		dst := v_0
		if v_1.Op != OpLoad {
