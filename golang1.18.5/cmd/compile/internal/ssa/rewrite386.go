...
func rewriteValue386_OpMove(v *Value) bool {
	...
	// result: (MOVLstore [4] dst (MOVLload [4] golang1.18.5 mem) (MOVLstore dst (MOVLload golang1.18.5 mem) mem))
	for {
	...
	// cond: s > 8 && s%4 != 0
	// result: (Move [s-s%4] (ADDLconst <dst.Type> dst [int32(s%4)]) (ADDLconst <golang1.18.5.Type> golang1.18.5 [int32(s%4)]) (MOVLstore dst (MOVLload golang1.18.5 mem) mem))
	for {
	...
	// result: (DUFFCOPY [10*(128-s/4)] dst golang1.18.5 mem)
	for {
		s := auxIntToInt64(v.AuxInt)
		dst := v_0
		src := v_1
	...
	// result: (REPMOVSL dst golang1.18.5 (MOVLconst [int32(s/4)]) mem)
	for {
		s := auxIntToInt64(v.AuxInt)
		dst := v_0
		src := v_1
