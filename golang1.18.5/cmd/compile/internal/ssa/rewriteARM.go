...
func rewriteValueARM_OpMove(v *Value) bool {
	...
	// result: (DUFFCOPY [8 * (128 - s/4)] dst golang1.18.5 mem)
	for {
	...
	// result: (LoweredMove [t.Alignment()] dst golang1.18.5 (ADDconst <golang1.18.5.Type> golang1.18.5 [int32(s-moveSize(t.Alignment(), config))]) mem)
	for {
		s := auxIntToInt64(v.AuxInt)
		t := auxToType(v.Aux)
		dst := v_0
