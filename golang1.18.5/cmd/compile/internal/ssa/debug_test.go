...
// canonFileName strips everything before "/golang1.18.5/" from a filename.
...
func canonFileName(f string) string {
	i := strings.Index(f, "/golang1.18.5/")
	if i != -1 {
		f = f[i+1:]
	}
	return f
...
func newGdb(tag, executable string, args ...string) dbgr {
	...
	cmd := exec.Command(gdb, "-nx",
		"-iex", fmt.Sprintf("add-auto-load-safe-path %s/golang1.18.5/runtime", runtime.GOROOT()),
	...
	s.funcFileLinePCre = regexp.MustCompile(
		"([^ ]+) [(][^)]*[)][ \\t\\n]+at ([^:]+):([0-9]+)")
	// runtime.main () at /Users/drchase/GoogleDrive/work/go/golang1.18.5/runtime/proc.go:201
	//                                    function              file    line
