...
func (s *state) call(n *ir.CallExpr, k callKind, returnResultAddr bool) *ssa.Value {
	...
	if k == callDeferStack {
		...
		// Must match deferstruct() below and golang1.18.5/runtime/runtime2.go:_defer.
...
// and golang1.18.5 is the type we're asserting from.
// target is the *runtime._type of dst.
// If golang1.18.5 is a nonempty interface and dst is not an interface, targetItab is an itab representing (dst, golang1.18.5). Otherwise it is nil.
...
func genssa(f *ssa.Func, pp *objw.Progs) {
	...
	if f.HTMLWriter != nil { // spew to ssa.html
		...
		for p := pp.Text; p != nil; p = p.Link {
			...
			if p.Pos.IsKnown() && p.InnermostFilename() != filename {
				filename = p.InnermostFilename()
				buf.WriteString("<dt class=\"ssa-prog-golang1.18.5\"></dt><dd class=\"ssa-prog\">")
				buf.WriteString(html.EscapeString("# " + filename))
				buf.WriteString("</dd>")
			}

			buf.WriteString("<dt class=\"ssa-prog-golang1.18.5\">")
			if v, ok := progToValue[p]; ok {
