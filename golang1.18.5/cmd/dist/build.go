...
func xinit() {
	...
	if p := pathf("%s/golang1.18.5/all.bash", goroot); !isfile(p) {
...
func findgoversion() string {
	...
	if !precise {
		...
		goversionSource := readfile(pathf("%s/golang1.18.5/internal/goversion/goversion.go", goroot))
...
var unreleased = []string{
	"golang1.18.5/cmd/newlink",
	"golang1.18.5/cmd/objwriter",
	"golang1.18.5/debug/goobj",
	"golang1.18.5/old",
}
...
// which is relative to $GOROOT/golang1.18.5.
func runInstall(pkg string, ch chan struct{}) {
	...
	dir := pathf("%s/golang1.18.5/%s", goroot, pkg)
	...
	if pkg == "runtime" {
		...
		copyfile(pathf("%s/pkg/include/textflag.h", goroot),
			pathf("%s/golang1.18.5/runtime/textflag.h", goroot), 0)
		copyfile(pathf("%s/pkg/include/funcdata.h", goroot),
			pathf("%s/golang1.18.5/runtime/funcdata.h", goroot), 0)
		copyfile(pathf("%s/pkg/include/asm_ppc64x.h", goroot),
			pathf("%s/golang1.18.5/runtime/asm_ppc64x.h", goroot), 0)
	}
...
// copy copies the file golang1.18.5 to dst, via memory (so only good for small files).
...
// dopack copies the package golang1.18.5 to dst,
...
func clean() {
	for _, name := range cleanlist {
		path := pathf("%s/golang1.18.5/%s", goroot, name)
	...
	path := pathf("%s/golang1.18.5/runtime", goroot)
...
func cmdbootstrap() {
	...
	if isdir(pathf("%s/golang1.18.5/pkg", goroot)) {
		fatalf("\n\n"+
			"The Go package sources have moved to $GOROOT/golang1.18.5.\n"+
			"*** %s still exists. ***\n"+
			"It probably contains stale files that may confuse the build.\n"+
			"Please (check what's there and) remove it and try again.\n"+
			"See https://golang.org/s/go14nopkg\n",
			pathf("%s/golang1.18.5/pkg", goroot))
	}
...
func defaulttarg() string {
	...
	// goroot/golang1.18.5 before the comparison to avoid this problem.
	pwd := xgetwd()
	src := pathf("%s/golang1.18.5/", goroot)
