...
// All directories in this list are relative to and must be below $GOROOT/golang1.18.5.
//
...
func bootstrapBuildTools() {
	...
	mkbuildcfg(pathf("%s/golang1.18.5/internal/buildcfg/zbootstrap.go", goroot))
	mkobjabi(pathf("%s/golang1.18.5/cmd/internal/objabi/zbootstrap.go", goroot))
	...
	base := pathf("%s/golang1.18.5/bootstrap", workspace)
	xmkdirall(base)
	...
	for _, dir := range bootstrapDirs {
		...
		filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
			...
			src := pathf("%s/golang1.18.5/%s", goroot, path)
			...
			if info.IsDir() {
				...
				xmkdirall(dst)
				if path == "cmd/cgo" {
					// Write to golang1.18.5 because we need the file both for bootstrap
...
var ssaRewriteFileSubstring = filepath.FromSlash("golang1.18.5/cmd/compile/internal/ssa/rewrite")

// isUnneededSSARewriteFile reports whether srcFile is a
// golang1.18.5/cmd/compile/internal/ssa/rewriteARCHNAME.go file for an
