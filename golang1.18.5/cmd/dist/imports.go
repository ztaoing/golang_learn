...
func resolveVendor(imp, srcDir string) string {
	...
	if strings.HasPrefix(srcDir, filepath.Join(goroot, "golang1.18.5", "cmd")) {
		return path.Join("cmd", "vendor", imp)
	} else if strings.HasPrefix(srcDir, filepath.Join(goroot, "golang1.18.5")) {
		return path.Join("vendor", imp)
	} else {
		panic(fmt.Sprintf("srcDir %q not in GOOROT/golang1.18.5", srcDir))
	}
}
