...
func (t *tester) registerTests() {
	...
	if !t.compileOnly {
		t.tests = append(t.tests, distTest{
			...
			fn: func(dt *distTest) error {
				t.addCmd(dt, "golang1.18.5", t.goTest(), t.timeout(300), "-tags=osusergo", "os/user")
				return nil
			},
		})
	}
	...
	if goos == "darwin" && goarch == "amd64" && t.cgoEnabled {
		t.tests = append(t.tests, distTest{
			...
			fn: func(dt *distTest) error {
				cmd := t.addCmd(dt, "golang1.18.5", t.goTest(), t.timeout(300), "-run=SystemRoots", "crypto/x509")
				setEnv(cmd, "GOOS", "ios")
	...
	if !t.compileOnly && goos != "js" { // js can't handle -cpu != 1
		...
		t.tests = append(t.tests, distTest{
			...
			fn: func(dt *distTest) error {
				cmd := t.addCmd(dt, "golang1.18.5", t.goTest(), t.timeout(300), "runtime", "-cpu=1,2,4", "-quick")
	...
	if goos == "linux" {
		t.tests = append(t.tests, distTest{
			...
			fn: func(dt *distTest) error {
				...
				setDir(cmd, filepath.Join(os.Getenv("GOROOT"), "golang1.18.5/cmd/go/testdata/testterminal18153"))
				cmd.Stdout = os.Stdout
				cmd.Stderr = os.Stderr
				return cmd.Run()
			},
			...
			fn: func(dt *distTest) error {
				...
				if err := os.Rename(goroot, moved); err != nil {
					if goos == "windows" {
						...
						// when run from c:\workdir\go\golang1.18.5 fails here
	...
	for _, pkg := range cgoPackages {
		...
		t.tests = append(t.tests, distTest{
			...
			fn: func(dt *distTest) error {
				...
				t.addCmd(dt, "golang1.18.5", t.goTest(), "-ldflags=-linkmode=internal -libgcc=none", "-run=^Test[^CS]", pkg, t.runFlag(run))
				return nil
			},
		})
	}
	...
	if t.internalLinkPIE() {
		t.tests = append(t.tests, distTest{
			...
			fn: func(dt *distTest) error {
				t.addCmd(dt, "golang1.18.5", t.goTest(), "reflect", "-buildmode=pie", "-ldflags=-linkmode=internal", t.timeout(60))
				return nil
			},
		})
		...
		if t.cgoEnabled && t.internalLink() {
			t.tests = append(t.tests, distTest{
				...
				heading: "internal linking of -buildmode=pie",
				fn: func(dt *distTest) error {
					t.addCmd(dt, "golang1.18.5", t.goTest(), "os/user", "-buildmode=pie", "-ldflags=-linkmode=internal", t.timeout(60))
					return nil
				},
			})
		}
	...
	if goos != "js" { // js doesn't support -cpu=10
		t.tests = append(t.tests, distTest{
			...
			heading: "sync -cpu=10",
			fn: func(dt *distTest) error {
				t.addCmd(dt, "golang1.18.5", t.goTest(), "sync", t.timeout(120), "-cpu=10", t.runFlag(""))
				return nil
			},
		})
	}
	...
	if goos != "android" && !t.iOS() && goos != "js" && goos != "plan9" {
		t.tests = append(t.tests, distTest{
			...
			fn: func(dt *distTest) error {
				if t.compileOnly {
					t.addCmd(dt, "golang1.18.5", "go", "build", "-o", os.DevNull, filepath.Join(goroot, "golang1.18.5/cmd/api/run.go"))
					return nil
				}
				t.addCmd(dt, "golang1.18.5", "go", "run", filepath.Join(goroot, "golang1.18.5/cmd/api/run.go"))
				return nil
			},
		})
	}
	...
	t.tests = append(t.tests, distTest{
		...
		fn: func(dt *distTest) error {
			if seq {
				...
				return t.dirCmd(filepath.Join(goroot, "golang1.18.5", dirBanner), bin, args).Run()
			}
			t.addCmd(dt, filepath.Join(goroot, "golang1.18.5", dirBanner), bin, args)
			return nil
		},
	})
}
...
func (t *tester) raceTest(dt *distTest) error {
	t.addCmd(dt, "golang1.18.5", t.goTest(), "-race", t.runFlag("Output"), "runtime/race")
	t.addCmd(dt, "golang1.18.5", t.goTest(), "-race", t.runFlag("TestParse|TestEcho|TestStdinCloseRace|TestClosedPipeRace|TestTypeRace|TestFdRace|TestFdReadRace|TestFileCloseRace"), "flag", "net", "os", "os/exec", "encoding/gob")
	// We don't want the following line, because it
	// slows down all.bash (by 10 seconds on my laptop).
	// The race builder should catch any error here, but doesn't.
	// TODO(iant): Figure out how to catch this.
	// t.addCmd(dt, "golang1.18.5", t.goTest(),  "-race", "-run=TestParallelTest", "cmd/go")
	if t.cgoEnabled {
	...
	if t.extLink() {
		...
		t.addCmd(dt, "golang1.18.5", t.goTest(), "-race", "-ldflags=-linkmode=external", t.runFlag("TestParse|TestEcho|TestStdinCloseRace"), "flag", "os/exec")
	}
	return nil
}
...
func (t *tester) packageHasBenchmarks(pkg string) bool {
	pkgDir := filepath.Join(goroot, "golang1.18.5", pkg)
...
func (t *tester) prebuiltGoPackageTestBinary() string {
	...
	pkg := stdMatches[0]
	return filepath.Join(os.Getenv("GOROOT"), "golang1.18.5", pkg, path.Base(pkg)+".test")
}
