	...
	data := packageDataCache.Do(r.path, func() any {
		...
		if r.dir != "" {
			...
			if cfg.ModulesEnabled {
				...
				// the module is inside $GOPATH/golang1.18.5.
...
func dirAndRoot(path string, dir, root string) (string, string) {
	...
	root = filepath.Join(root, "golang1.18.5")
	...
	if !str.HasFilePathPrefix(dir, root) || len(dir) <= len(root) || dir[len(root)] != filepath.Separator || path != "command-line-arguments" && !build.IsLocalImport(path) && filepath.Join(root, path) != dir {
		...
		base.Fatalf("unexpected directory layout:\n"+
			...
			"	separator: %s",
			path,
			filepath.Join(origRoot, "golang1.18.5"),
...
func vendoredImportPath(path, parentPath, parentDir, parentRoot string) (found string) {
	...
	for i := len(dir); i >= len(root); i-- {
		...
		if isDir(targ) && hasGoFiles(targ) {
			...
			// We started with parent's dir c:\gopath\golang1.18.5\foo\bar\baz\quux\xyzzy.
			// We know the import path for parent's dir.
			// We chopped off some number of path elements and
			// added vendor\path to produce c:\gopath\golang1.18.5\foo\bar\baz\vendor\path.
			...
			if chopped == len(importPath)+1 {
				// We walked up from c:\gopath\golang1.18.5\foo\bar
				// and found c:\gopath\golang1.18.5\vendor\path.
...
HaveGoMod:
	...
	// Allow it to refer to code in GOPATH/golang1.18.5/x/y/z as x/y/v2/z
	// if GOPATH/golang1.18.5/x/y/go.mod says module "x/y/v2",
