...
GOPATH/golang1.18.5/<import-path>. If the GOPATH contains multiple entries,
...
func download(arg string, parent *load.Package, stk *load.ImportStack, mode int) {
	...
	//   mkdir -p $GOPATH/golang1.18.5/github.com/user
	//   cd $GOPATH/golang1.18.5/github.com/user
	//   go get ./foo
...
func downloadPackage(p *load.Package) error {
	...
	if p.Internal.Build.SrcRoot != "" {
		// Directory exists. Look for checkout along path to golang1.18.5.
	...
	if p.Internal.Build.SrcRoot == "" {
		...
		if _, err := os.Stat(filepath.Join(list[0], "golang1.18.5/cmd/go/alldocs.go")); err == nil {
			return fmt.Errorf("cannot download, %s is a GOROOT, not a GOPATH. For more details see: 'go help gopath'", list[0])
		}
		p.Internal.Build.Root = list[0]
		p.Internal.Build.SrcRoot = filepath.Join(list[0], "golang1.18.5")
