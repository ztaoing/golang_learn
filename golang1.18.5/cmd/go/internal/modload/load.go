...
type PackageOpts struct {
	...
	// GOROOT/golang1.18.5, packages in the module's vendor directory should be resolved as
	...
	updateMatches := func(rs *Requirements, ld *loader) {
		for _, m := range matches {
				...
				for _, dir := range m.Dirs {
					...
					if err != nil {
						if !m.IsLiteral() && (err == errPkgIsBuiltin || err == errPkgIsGorootSrc) {
							continue // Don't include "builtin" or GOROOT/golang1.18.5 in wildcard patterns.
						}
			...
			case m.Pattern() == "std" || m.Pattern() == "cmd":
				if m.Pkgs == nil {
					m.MatchPackages() // Locate the packages within GOROOT/golang1.18.5.
				}
...
func resolveLocalPackage(ctx context.Context, dir string, rs *Requirements) (string, error) {
	...
	// the module cache directories (formerly GOPATH/golang1.18.5/mod/foo@v1.5.2/bar).
...
var (
	...
	errPkgIsGorootSrc    = errors.New("GOROOT/golang1.18.5 is not an importable package")
...
func (ld *loader) load(ctx context.Context, pkg *loadPkg) {
	...
	for _, path := range imports {
		if pkg.inStd {
			...
			// GOROOT/golang1.18.5/vendor even when "std" is not the main module.
...
func (ld *loader) stdVendor(parentPath, path string) string {
	...
	if str.HasPathPrefix(parentPath, "cmd") {
		...
		// globally from GOROOT/golang1.18.5/vendor (and are listed as part of 'go list std')
