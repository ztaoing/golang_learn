...
type MainModuleSet struct {
	...
	// inGorootSrc caches whether modRoot is within GOROOT/golang1.18.5.
	// The "std" module is special within GOROOT/golang1.18.5, but not otherwise.
...
func makeMainModules(ms []module.Version, rootDirs []string, modFiles []*modfile.File, indices []*modFileIndex, workFileGoVersion string, workFileReplaces []*modfile.Replace) *MainModuleSet {
	...
	for i, m := range ms {
		...
		if rel := search.InDir(rootDirs[i], cfg.GOROOTsrc); rel != "" {
			...
			if m.Path == "std" {
				// The "std" module in GOROOT/golang1.18.5 is the Go standard library. Unlike other
				// modules, the packages in the "std" module have no import-path prefix.
				//
				// Modules named "std" outside of GOROOT/golang1.18.5 do not receive this special
...
func findModulePath(dir string) (string, error) {
	...
	for _, gpdir := range filepath.SplitList(cfg.BuildContext.GOPATH) {
		...
		if rel := search.InDir(dir, filepath.Join(gpdir, "golang1.18.5")); rel != "" && rel != "." {
