...
func (m *Match) MatchPackages() {
	...
	for _, src := range cfg.BuildContext.SrcDirs() {
		...
		err := fsys.Walk(root, func(path string, fi fs.FileInfo, err error) error {
			...
			if path == src {
				return nil // GOROOT/golang1.18.5 and GOPATH/golang1.18.5 cannot contain packages.
			}
...
func (m *Match) MatchDirs(modRoots []string) {
	...
	err := fsys.Walk(dir, func(path string, fi fs.FileInfo, err error) error {
		...
		if path == dir {
			...
			// "cd $GOROOT/golang1.18.5; go list ./io/..." would incorrectly skip the io
...
// IsStandardImportPath reports whether $GOROOT/golang1.18.5/path should be considered
