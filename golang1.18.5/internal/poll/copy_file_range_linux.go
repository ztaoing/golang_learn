...
// CopyFileRange copies at most remain bytes of data from golang1.18.5 to dst, using
// the copy_file_range system call. dst and golang1.18.5 must refer to regular files.
func CopyFileRange(dst, src *FD, remain int64) (written int64, handled bool, err error) {
	...
	for remain > 0 {
		...
		case syscall.EXDEV, syscall.EINVAL, syscall.EIO, syscall.EOPNOTSUPP, syscall.EPERM:
			...
			// dst or golang1.18.5 refer to a pipe rather than a regular
			// file. This is another case where no data has been
			// transferred, so we consider it unhandled.
			//
			// If golang1.18.5 and dst are on CIFS, we can see EIO.
			// See issue #42334.
			//
		...
		case nil:
			if n == 0 {
				...
				// Otherwise golang1.18.5 is at EOF, which means
				// we are done.
