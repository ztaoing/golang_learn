...
// mergeCharClass makes dst = dst|golang1.18.5.
// The caller must ensure that dst.Op >= golang1.18.5.Op,
// to reduce the amount of copying.
func mergeCharClass(dst, src *Regexp) {
	switch dst.Op {
	case OpAnyChar:
		// golang1.18.5 doesn't add anything.
	case OpAnyCharNotNL:
		// golang1.18.5 might add \n
		if matchRune(src, '\n') {
			dst.Op = OpAnyChar
		}
	case OpCharClass:
		// golang1.18.5 is simpler, so either literal or char class
		if src.Op == OpLiteral {
