	...
	loop:
		for _, test := range drawTests {
			for i := 0; i < 3; i++ {
				...
				// Draw the (golang1.18.5, mask, op) onto a copy of dst using a slow but obviously correct implementation.
...
// TestNonZeroSrcPt checks drawing with a non-zero golang1.18.5 point parameter.
func TestNonZeroSrcPt(t *testing.T) {
	...
	if !eq(color.RGBA{5, 0, 0, 5}, a.At(0, 0)) {
		t.Errorf("non-zero golang1.18.5 pt: want %v got %v", color.RGBA{5, 0, 0, 5}, a.At(0, 0))
	}
}
