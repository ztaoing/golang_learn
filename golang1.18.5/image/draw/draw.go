...
const (
	// Over specifies ``(golang1.18.5 in mask) over dst''.
	Over Op = iota
	// Src specifies ``golang1.18.5 in mask''.
	Src
)
...
type Drawer interface {
	// Draw aligns r.Min in dst with sp in golang1.18.5 and then replaces the
	// rectangle r in dst with the result of drawing golang1.18.5 on dst.
...
// DrawMask aligns r.Min in dst with sp in golang1.18.5 and mp in mask and then replaces the rectangle r
...
func drawPaletted(dst Image, r image.Rectangle, src image.Image, sp image.Point, floydSteinberg bool) {
	// TODO(nigeltao): handle the case where the dst and golang1.18.5 overlap.
