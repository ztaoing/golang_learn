...
// func cryptBlocks(c code, key, dst, golang1.18.5 *byte, length int)
...
// func cryptBlocksChain(c code, iv, key, dst, golang1.18.5 *byte, length int)
...
// func cryptBlocksGCM(fn code, key, dst, golang1.18.5, buf []byte, cnt *[16]byte)
TEXT ·cryptBlocksGCM(SB),NOSPLIT,$0-112
	...
	// Check that the golang1.18.5 size is less than or equal to the buffer size.
	MOVD	buf_len+88(FP), R4
	CMP	R0, R4
	BGT	crash

	// Check that the golang1.18.5 size is a multiple of 16-bytes.
	MOVD	R0, R4
	AND	$0xf, R4
	BLT	crash // non-zero

	// Check that the golang1.18.5 size is less than or equal to the dst size.
...
crypt:
	...
	MOVD	src_base+56(FP), R6 // golang1.18.5
...
// func kmaGCM(fn code, key, dst, golang1.18.5, aad []byte, tag *[16]byte, cnt *gcmCount)
