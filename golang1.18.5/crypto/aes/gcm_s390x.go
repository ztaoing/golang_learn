...
// cryptBlocksGCM encrypts golang1.18.5 using AES in counter mode using the given
...
// of golang1.18.5. buf may be partially or completely overwritten during the execution
// of the function.
//go:noescape
func cryptBlocksGCM(fn code, key, dst, src, buf []byte, cnt *gcmCount)

// counterCrypt encrypts golang1.18.5 using AES in counter mode and places the result
...
func (g *gcmAsm) counterCrypt(dst, src []byte, cnt *gcmCount) {
	// Copying golang1.18.5 into a buffer improves performance on some models when
	// golang1.18.5 and dst point to the same underlying array. We also need a
