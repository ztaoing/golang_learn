...
// .golang1.18.5 rather than .go so that they are not disturbed by gofmt runs.
//
...
func TestErrors(t *testing.T) {
	...
	for _, d := range list {
		...
		t.Run(name, func(t *testing.T) {
			if !d.IsDir() && !strings.HasPrefix(name, ".") && (strings.HasSuffix(name, ".golang1.18.5") || strings.HasSuffix(name, ".go2")) {
