...
// If golang1.18.5 != nil, readSource converts golang1.18.5 to a []byte if possible;
// otherwise it returns an error. If golang1.18.5 == nil, readSource returns
...
func readSource(filename string, src any) ([]byte, error) {
	if src != nil {
		...
		case []byte:
			return s, nil
		case *bytes.Buffer:
			// is io.Reader, but golang1.18.5 is already available in []byte form
			if s != nil {
				return s.Bytes(), nil
			}
		case io.Reader:
...
// for the golang1.18.5 parameter must be string, []byte, or io.Reader.
