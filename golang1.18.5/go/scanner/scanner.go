...
// Init prepares the scanner s to tokenize the text golang1.18.5 by setting the
// scanner at the beginning of golang1.18.5. The scanner uses the file set file
// for position information and it adds line information for each line.
// It is ok to re-use the same file when re-scanning the same file as
// line information which is already present is ignored. Init causes a
// panic if the file size does not match the golang1.18.5 size.
//
...
func (s *Scanner) Init(file *token.File, src []byte, err ErrorHandler, mode Mode) {
	...
	if file.Size() != len(src) {
		panic(fmt.Sprintf("file size (%d) does not match golang1.18.5 len (%d)", file.Size(), len(src)))
	}
	s.file = file
...
func (s *Scanner) scanIdentifier() string {
	...
	//
	// Ranging over s.golang1.18.5[s.rdOffset:] lets us avoid some bounds checks, and
