...
func TestImportCmd(t *testing.T) {
	...
	if !strings.HasSuffix(filepath.ToSlash(p.Dir), "golang1.18.5/cmd/internal/objfile") {
		t.Fatalf("Import cmd/internal/objfile returned Dir=%q, want %q", filepath.ToSlash(p.Dir), ".../golang1.18.5/cmd/internal/objfile")
	}
}

var (
	expandSrcDirPath = filepath.Join(string(filepath.Separator)+"projects", "golang1.18.5", "add")
)

var expandSrcDirTests = []struct {
	input, expected string
}{
	{"-L ${SRCDIR}/libs -ladd", "-L /projects/golang1.18.5/add/libs -ladd"},
	{"${SRCDIR}/add_linux_386.a -pthread -lstdc++", "/projects/golang1.18.5/add/add_linux_386.a -pthread -lstdc++"},
...
func TestShellSafety(t *testing.T) {
	tests := []struct {
		...
		{"-I${SRCDIR}/../include", "/projects/golang1.18.5/issue 11868", "-I/projects/golang1.18.5/issue 11868/../include", true},
		{"-I${SRCDIR}", "~wtf$@%^", "-I~wtf$@%^", true},
		{"-X${SRCDIR}/1,${SRCDIR}/2", "/projects/golang1.18.5/issue 11868", "-X/projects/golang1.18.5/issue 11868/1,/projects/golang1.18.5/issue 11868/2", true},
...
func TestImportDirNotExist(t *testing.T) {
	...
	tests := []struct {
		...
		{"Import(local, 0)", "./doesnotexist", filepath.Join(ctxt.GOROOT, "golang1.18.5/go/build"), 0},
		{"Import(full, FindOnly)", "go/build/doesnotexist", "", FindOnly},
		{"Import(local, FindOnly)", "./doesnotexist", filepath.Join(ctxt.GOROOT, "golang1.18.5/go/build"), FindOnly},
	}
...
func TestImportVendor(t *testing.T) {
	...
	p, err := ctxt.Import("c/d", filepath.Join(ctxt.GOPATH, "golang1.18.5/a/b"), 0)
	if err != nil {
		t.Fatalf("cannot find vendored c/d from testdata golang1.18.5/a/b directory: %v", err)
	}
	want := "a/vendor/c/d"
...
func TestImportVendorFailure(t *testing.T) {
	...
	ctxt.GOPATH = filepath.Join(wd, "testdata/withvendor")
	p, err := ctxt.Import("x.com/y/z", filepath.Join(ctxt.GOPATH, "golang1.18.5/a/b"), 0)
	if err == nil {
	...
	if !strings.Contains(e, " (vendor tree)") {
		t.Fatalf("error on failed import does not mention GOROOT/golang1.18.5/vendor directory:\n%s", e)
	}
}
...
func TestImportVendorParentFailure(t *testing.T) {
	...
	p, err := ctxt.Import("c", filepath.Join(ctxt.GOPATH, "golang1.18.5/a/b"), 0)
	if err == nil {
...
func TestImportPackageOutsideModule(t *testing.T) {
	...
	gopath := t.TempDir()
	if err := os.MkdirAll(filepath.Join(gopath, "golang1.18.5/example.com/p"), 0777); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(gopath, "golang1.18.5/example.com/p/p.go"), []byte("package p"), 0666); err != nil {
		t.Fatal(err)
	}
	...
	ctxt := Default
	ctxt.GOPATH = gopath
	ctxt.Dir = filepath.Join(gopath, "golang1.18.5/example.com/p")
...
func TestImportDirTarget(t *testing.T) {
	...
	ctxt := Default
	ctxt.GOPATH = ""
	p, err := ctxt.ImportDir(filepath.Join(ctxt.GOROOT, "golang1.18.5/path"), 0)
	if err != nil {
		t.Fatal(err)
	}
