...
func (ctxt *Context) SrcDirs() []string {
	...
	if ctxt.GOROOT != "" && ctxt.Compiler != "gccgo" {
		dir := ctxt.joinPath(ctxt.GOROOT, "golang1.18.5")
		if ctxt.isDir(dir) {
			all = append(all, dir)
		}
	}
	for _, p := range ctxt.gopath() {
		dir := ctxt.joinPath(p, "golang1.18.5")
		if ctxt.isDir(dir) {
			all = append(all, dir)
		}
	}
...
func (ctxt *Context) Import(path string, srcDir string, mode ImportMode) (*Package, error) {
	...
	if IsLocalImport(path) {
		...
		if ctxt.GOROOT != "" {
			root := ctxt.joinPath(ctxt.GOROOT, "golang1.18.5")
		...
		for i, root := range all {
			rootsrc := ctxt.joinPath(root, "golang1.18.5")
			if sub, ok := ctxt.hasSubdir(rootsrc, p.Dir); ok && !inTestdata(sub) {
				// We found a potential import path for dir,
				// but check that using it wouldn't find something
				// else first.
				if ctxt.GOROOT != "" && ctxt.Compiler != "gccgo" {
					if dir := ctxt.joinPath(ctxt.GOROOT, "golang1.18.5", sub); ctxt.isDir(dir) {
						p.ConflictDir = dir
						goto Found
					}
				}
				for _, earlyRoot := range all[:i] {
					if dir := ctxt.joinPath(earlyRoot, "golang1.18.5", sub); ctxt.isDir(dir) {
						p.ConflictDir = dir
						goto Found
					}
				}
		...
		if mode&IgnoreVendor == 0 && srcDir != "" {
			searchVendor := func(root string, isGoroot bool) bool {
				...
				if !ok || !strings.HasPrefix(sub, "golang1.18.5/") || strings.Contains(sub, "/testdata/") {
					return false
				}
				for {
					...
					if ctxt.isDir(vendor) {
						...
						if ctxt.isDir(dir) && hasGoFiles(ctxt, dir) {
							p.Dir = dir
							p.ImportPath = strings.TrimPrefix(pathpkg.Join(sub, "vendor", path), "golang1.18.5/")
							p.Goroot = isGoroot
							p.Root = root
							setPkga() // p.ImportPath changed
							return true
		...
		if ctxt.GOROOT != "" {
			...
			if gorootFirst {
				dir := ctxt.joinPath(ctxt.GOROOT, "golang1.18.5", path)
		...
		if ctxt.Compiler == "gccgo" && goroot.IsStandardPackage(ctxt.GOROOT, ctxt.Compiler, path) {
			p.Dir = ctxt.joinPath(ctxt.GOROOT, "golang1.18.5", path)
			p.Goroot = true
			p.Root = ctxt.GOROOT
			goto Found
		}
		for _, root := range gopath {
			dir := ctxt.joinPath(root, "golang1.18.5", path)
			isDir := ctxt.isDir(dir)
...
Found:
	if p.Root != "" {
		p.SrcRoot = ctxt.joinPath(p.Root, "golang1.18.5")
...
func (ctxt *Context) importGo(p *Package, path, srcDir string, mode ImportMode) error {
	...
	if srcDir != "" {
		...
		if _, ok := ctxt.hasSubdir(filepath.Join(ctxt.GOROOT, "golang1.18.5"), absSrcDir); ok {
			return errNoModules
		}
	}
