...
// format parses golang1.18.5, prints the corresponding AST, verifies the resulting
// golang1.18.5 is syntactically correct, and returns the resulting golang1.18.5 or an error
// if any.
...
`
	...
	// parse original
	f1, err := parser.ParseFile(fset, "golang1.18.5", src, parser.ParseComments)
	if err != nil {
		t.Fatal(err)
	}
...
`
	...
	const want = `//line golang1.18.5.go:2
package p

//line golang1.18.5.go:3
func f() {}

var x, y, z int

//line golang1.18.5.go:8
func g() {
}
`

	// parse original
	f1, err := parser.ParseFile(fset, "golang1.18.5.go", orig, 0)
	if err != nil {
		t.Fatal(err)
	}
