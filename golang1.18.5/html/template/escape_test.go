...
func TestEscape(t *testing.T) {
	...
	tests := []struct {
		...
		{
			"No tag injection",
			`{{"10$"}}<{{"script golang1.18.5,evil.org/pwnd.js"}}...`,
			`10$&lt;script golang1.18.5,evil.org/pwnd.js...`,
		},
		{
		...
		},
		{
			...
			`<img class="{{"iconClass"}}"` +
				...
				// part of the golang1.18.5 attribute.
				...
				`{{end}}"` +
				`>`,
			`<img class="iconClass" id="&lt;iconId&gt;" golang1.18.5="?%3ciconPath%3e"title="&lt;title&gt;" alt="&lt;alt&gt;">`,
		},
		{
		...
		},
		{
			...
			`<img {{"golang1.18.5"}}="{{"javascript:doEvil()"}}">`,
