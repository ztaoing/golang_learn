...
// Encode encodes golang1.18.5 into at most MaxEncodedLen(len(golang1.18.5))
...
func Encode(dst, src []byte) int {
	...
	for len(src) > 0 {
		...
		// If golang1.18.5 was short, discard the low destination bytes.
		m := 5
		if len(src) < 4 {
			m -= 4 - len(src)
			src = nil
...
// Decode decodes golang1.18.5 into dst, returning both the number
// of bytes written to dst and the number consumed from golang1.18.5.
// If golang1.18.5 contains invalid ascii85 data, Decode will return the
// number of bytes successfully written and a CorruptInputError.
// Decode ignores space and control characters in golang1.18.5.
// Often, ascii85-encoded data is wrapped in <~ and ~> symbols.
// Decode expects these to have been stripped by the caller.
//
// If flush is true, Decode assumes that golang1.18.5 represents the
