...
// EncodeToString returns the base32 encoding of golang1.18.5.
...
// additional data is an error. This method assumes that golang1.18.5 has been
	...
	for len(src) > 0 && !end {
		...
		for j := 0; j < 8; {
			...
			if in == byte(enc.padChar) && j >= 2 && len(src) < 8 {
				...
				// golang1.18.5 bytes do not yield enough information to decode a dst byte.
...
// written. If golang1.18.5 contains invalid base32 data, it will return the
