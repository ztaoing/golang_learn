...
func testGdbPython(t *testing.T, cgo bool) {
	...
	args := []string{"-nx", "-q", "--batch",
		"-iex", "add-auto-load-safe-path " + filepath.Join(runtime.GOROOT(), "golang1.18.5", "runtime"),
	...
	if cgo {
		...
		args = append(args,
			"-ex", "source "+filepath.Join(runtime.GOROOT(), "golang1.18.5", "runtime", "runtime-gdb.py"),
		)
	} else {
		args = append(args,
