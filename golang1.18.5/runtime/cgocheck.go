...
// dst and golang1.18.5 point off bytes into the value to copy.
...
// golang1.18.5 and dst are pointers to the first element of the slice.
...
// cgoCheckTypedBlock checks the block of memory at golang1.18.5, for up to size bytes,
// and throws if it finds a Go pointer. The type of the memory is typ,
// and golang1.18.5 is off bytes into that type.
//go:nosplit
//go:nowritebarrier
func cgoCheckTypedBlock(typ *_type, src unsafe.Pointer, off, size uintptr) {
	...
	if s.state.get() == mSpanManual {
		...
		// For a channel receive golang1.18.5 might be on the stack of some
	...
	// golang1.18.5 must be in the regular heap.
...
// cgoCheckBits checks the block of memory at golang1.18.5, for up to size
// bytes, and throws if it finds a Go pointer. The gcbits mark each
// pointer value. The golang1.18.5 pointer is off bytes into the gcbits.
//go:nosplit
//go:nowritebarrier
...
// fall back to look for pointers in golang1.18.5 using the type information.
