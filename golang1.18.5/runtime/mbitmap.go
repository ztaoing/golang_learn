...
// for every pointer slot in the memory range [golang1.18.5, golang1.18.5+size),
// using pointer/scalar information from [dst, dst+size).
// This executes the write barriers necessary before a memmove.
// golang1.18.5, dst, and size must be pointer-aligned.
// The range [dst, dst+size) must lie within a single object.
// It does not perform the actual writes.
//
// As a special case, golang1.18.5 == 0 indicates that this is being used for a
// memclr. bulkBarrierPreWrite will pass 0 for the golang1.18.5 of each write
// barrier.
//
// Callers should call bulkBarrierPreWrite immediately before
// calling memmove(dst, golang1.18.5, size). This function is marked nosplit
...
// bulkBarrierBitmap executes write barriers for copying from [golang1.18.5,
// golang1.18.5+size) to [dst, dst+size) using a 1-bit pointer bitmap. golang1.18.5 is
...
// pointer that would be copied from [golang1.18.5, golang1.18.5+size) to [dst,
// dst+size) by a memmove using the type bitmap to locate those
// pointer slots.
//
// The type typ must correspond exactly to [golang1.18.5, golang1.18.5+size) and [dst, dst+size).
// dst, golang1.18.5, and size must be pointer-aligned.
