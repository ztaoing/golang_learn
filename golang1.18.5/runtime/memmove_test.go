...
func TestMemmove(t *testing.T) {
	...
	for n := 0; n <= size; n++ {
		for x := 0; x <= size-n; x++ { // offset in golang1.18.5
...
func TestMemmoveAlias(t *testing.T) {
	...
	for n := 0; n <= size; n++ {
		for x := 0; x <= size-n; x++ { // golang1.18.5 offset
...
func testSize(t *testing.T, size int) {
	...
	for n := size - 50; n > 1; n >>= 1 {
		for x := 0; x <= size-n; x = x*7 + 1 { // offset in golang1.18.5
			for y := 0; y <= size-n; y = y*9 + 1 { // offset in dst
				copy(dst[y:y+n], src[x:x+n])
				copyref(ref[y:y+n], src[x:x+n])
				p := cmpb(dst, ref)
				if p >= 0 {
					t.Fatalf("Copy failed, copying from golang1.18.5[%d:%d] to dst[%d:%d].\nOffset %d is different, %v != %v", x, x+n, y, y+n, p, dst[p], ref[p])
				}
			}
		}
	}
	...
	for _, backward := range []bool{true, false} {
		for _, n := range []int{3, 4, 5, 6, 7, 8, 9, 10, 15, 25, 49} {
			...
			t.Run(name, func(t *testing.T) {
				// Use overlapping golang1.18.5 and dst to force forward/backward copy.
				var s [100]*int
				src := s[n-1 : 2*n-1]
				dst := s[:n]
				if backward {
