...
// encode encodes a block given in golang1.18.5 and appends tokens
...
func (e *deflateFast) encode(dst []token, src []byte) []token {
	...
	// nextEmit is where in golang1.18.5 the next emitLiteral should start from.
	nextEmit := int32(0)
	s := int32(0)
	cv := load32(src, s)
	nextHash := hash(cv)
	...
	for {
		...
		// match. But, prior to the match, golang1.18.5[nextEmit:s] are unmatched. Emit
...
// matchLen returns the match length between golang1.18.5[s:] and golang1.18.5[t:].
// t can be negative to indicate the match is starting in e.prev.
// We assume that golang1.18.5[s-4:s] and golang1.18.5[t-4:t] already match.
